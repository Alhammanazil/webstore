    ...
    public function makeAnOrder(CheckoutData $checkout_data): SalesOrderData
    {
        $sales_order = DB::transaction(function () use ($checkout_data) {
            ...
            $sales_order = SalesOrder::query()->create([
                ...
                // payment_method, payment_label, payment_payload, payment_paid_at
                ...
                'due_date_at' => Carbon::now()->addDays(7),
            ]);
            ...
            foreach ($checkout_data->cart->items as $item) {
                ...
                if ($product->stock < $item->quantity) {
                    throw new \Exception("Insufficient stock for product SKU: {$item->sku}");
                }
                ...
                $items->push([
                    ...
                    // short_dec, sku, slug, description, cover_url, quantity, price, total, weight
                    'short_dec' => $item->product()->short_desc,
                    'sku' => $item->sku,
